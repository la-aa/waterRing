import electron, { screen, app as app$1, Tray, nativeImage, Menu, ipcMain as ipcMain$1, BrowserWindow } from "electron";
...
const store = new ElectronStore({
  ...
  defaults: {
    ...
    nextReminderAt: Date.now() + DEFAULT_SETTINGS.intervalMinutes * 6e4
  }
});
const settingsStore = {
  getSettings() {
  ...
  setNextReminderAt(value) {
    store.set("nextReminderAt", value);
  }
};
function createTray(mainWindow2) {
  const iconPath = path.join(app$1.getAppPath(), "ico/myCat_ico.jpg");
  const tray = new Tray(nativeImage.createFromPath(iconPath).resize({ width: 18, height: 18 }));
  tray.setToolTip("waterRing");
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label: "显示小助手", click: () => mainWindow2.show() },
      { label: "隐藏", click: () => mainWindow2.hide() },
      { type: "separator" },
      { label: "退出", click: () => app$1.quit() }
    ])
  );
...
const userDataRoot = path.join(app$1.getPath("appData"), "waterRing");
const appRoot = app$1.getAppPath();
mkdirSync(path.join(userDataRoot, "Cache"), { recursive: true });
app$1.commandLine.appendSwitch("user-data-dir", userDataRoot);
app$1.commandLine.appendSwitch("disable-gpu");
app$1.commandLine.appendSwitch("disable-gpu-compositing");
app$1.setPath("userData", userDataRoot);
app$1.setPath("cache", path.join(userDataRoot, "Cache"));
function getPreloadPath() {
  return app$1.isPackaged ? path.join(process.resourcesPath, "out/preload/index.mjs") : path.join(appRoot, "out/preload/index.mjs");
}
function getIconPath() {
  return app$1.isPackaged ? path.join(process.resourcesPath, "ico/myCat_ico.jpg") : path.join(appRoot, "ico/myCat_ico.jpg");
}
...
app$1.whenReady().then(() => {
  createWindow();
  ipcMain$1.handle("settings:get", () => settingsStore.getSettings());
  ipcMain$1.handle("settings:set", (_event, next2) => {
    const merged = settingsStore.setSettings(next2);
    setAutoStart(merged.autoStart);
    syncReminderTimer();
    return merged;
  });
  ipcMain$1.handle("reminder:snooze", () => {
    const next2 = Date.now() + settingsStore.getSettings().snoozeMinutes * 6e4;
    settingsStore.setNextReminderAt(next2);
    return next2;
  });
  ipcMain$1.handle("reminder:confirm", () => {
    const next2 = Date.now() + settingsStore.getSettings().intervalMinutes * 6e4;
    settingsStore.setNextReminderAt(next2);
    return next2;
  });
  ipcMain$1.handle("reminder:getNext", () => settingsStore.getNextReminderAt());
});
app$1.on("activate", () => {
  if (!mainWindow) {
    createWindow();
